...
- `Playbook 关键牌执行索引` 优先服务战斗决策；若它与原 tier 文本冲突，先按当前 MCP 卡牌描述和执行索引判断，再把冲突写入 `02_核心设计文档/08_结构化复盘与规则沉淀.md`。

## Playbook 关键牌执行索引

> 本节补齐 `02_核心设计文档/04_铁甲战士策略手册.md` 已直接依赖、但原卡牌攻略没有完整结构化字段的关键牌。`cost/type` 若不是来自原攻略，使用 `verify_with_mcp_card_text` 提醒实战前以 MCP 当前卡牌描述为准。
...
source_docs: ["02_核心设计文档/08_结构化复盘与规则沉淀.md", "02_核心设计文档/04_铁甲战士策略手册.md"]
confidence: high
```
...
source_docs: ["02_核心设计文档/08_结构化复盘与规则沉淀.md", "02_核心设计文档/05_铁甲战士卡牌知识库.md"]
confidence: high
```
